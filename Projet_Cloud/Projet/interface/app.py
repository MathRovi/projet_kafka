...
st.set_page_config(page_title="Dashboard de Pomme dAPI", layout="wide")
...
# Ajout d'une 11ᵉ couleur "#8B0000" pour éviter l'erreur de taille si le DataFrame a 11 lignes
fruits_colors = [
    "#FF0000", "#FF4500", "#FFD700", "#ADFF2F", "#32CD32",
    "#006400", "#008000", "#7CFC00", "#FFFF00", "#FF6347",
    "#8B0000"  # <--- 11e couleur ajoutée
]

revenue_by_product_df["Couleur"] = fruits_colors[:len(revenue_by_product_df)]
...
if st.session_state.page == "intro":
    ...
    image_base64 = get_base64_image("pomme.jpg")
    ...
    st.markdown(
        """
        <div class="container">
            <h1 class="title">Bienvenue sur le Tableau de Bord Intéractif de votre boutique de Fruits et Légumes ! 😄</h1>
    ...
    st.title("🍎 Dashboard des Ventes de Fruits et Légumes 🍎")
    ...
    st.sidebar.title("🍋‍🟩 Liste des produits 🍋‍🟩")
    ...
    if not revenue_by_product_df.empty:
        ...
        chart2_updated = alt.Chart(revenue_by_product_df).mark_bar().encode(
            ...
            color=alt.Color(
                "% du Revenu Total:Q",
                scale=alt.Scale(
                    domain=[revenue_by_product_df["% du Revenu Total"].min(),
                            revenue_by_product_df["% du Revenu Total"].max()],
    ...
    categories = {
        "Fruits": ["Pommes", "Poires", "Clémentines", "Orange"],
        "Légumes": ["Carrottes", "Poireau"],
        "Autres": ["Jus de pomme", "Jus de cassis", "Miel", "Sac de patate"]
    }

    revenue_by_category = {}
    for category, products in categories.items():
        revenue_by_category[category] = revenue_by_product_df[
            revenue_by_product_df["Produit"].isin(products)
        ]["Revenu (€)"].sum()
    ...
    if not revenue_by_category_df.empty:
        christmas_pie_colors = ["#FFB6C1", "#FF69B4", "#8B0000"]  # Adding new fruits-themed colors
    ...
    st.subheader("📒 Revenus et Ventes par Jour 📒")
